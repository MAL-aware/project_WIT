...
c = conn.cursor()
connguest = sqlite3.connect('dataguest.db', check_same_thread=False)
cg = connguest.cursor()
...
def page3():
    ...
    st.image(image, caption='Sumber: https://www.pngitem.com')
...
def page5():
    ...
    def create_table_guest():
        cg.execute('CREATE TABLE IF NOT EXISTS blogtableguest(author TEXT,title TEXT,article TEXT,postdate DATE)')
    ...
    def add_data_guest(author, title, article, postdate):
        cg.execute('INSERT INTO blogtableguest(author, title, article, postdate) VALUES(?,?,?,?)',(author, title, article, postdate))
        connguest.commit()
        ...
        if choice == "Tambah Artikel (Tamu)":
            ...
            with st.container():
                ...
                if st.button("Tambah"):
                    ...
                    st.success("Artikel: {} disimpan".format(blog_title))

        elif choice == "Tambah Artikel (Admin)":
            if check_password():
                st.markdown("<h3 style='text-align: center; color: yellow;'>Tambah Artikel dari Admin 📝</h3>", unsafe_allow_html=True)
                st.write(" ")
                with st.container():
                    st.markdown("<h3 style='text-align: left; color: lime;'>Tambah Artikel</h3>", unsafe_allow_html=True)
                    create_table()
